...
方案方向。AI 可以预填 3-5 个方案方向，必须标 `AI 草稿，待确认`；专家反馈绑定到"待验证假设"
或"筛选标准"，而不是写总评。
...
生成要点：先创建四个 root，再填 seed；每个 seed 都标来源或 `AI 草稿，待确认`。若某阶段无证据，
不要空着，写 `未发现 / 待补：需要补充...`。专家反馈绑定到具体 slot，例如 Persona、HMW、
