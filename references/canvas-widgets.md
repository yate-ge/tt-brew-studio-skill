...
- **请求型 Widget 同时写 state 和 emit event**：用户点击提交时，Widget 应先
  ...
  显式处理信号。
- **交互后的视觉状态**：agent 创建的 Widget 边框始终是黄色；只要用户在 Widget 内输入、
  改变状态或显式提交，运行时写入 `vd_widget_pending_feedback=true`。这表示该 Widget 已产生
  用户反馈内容，等待下一轮 agent 处理；不要把 Widget 边框改成紫色。
...
- **智能体回写状态**：处理请求前先用 `update_widget` 把 Widget 置为
  ...
  `update_widget` 成功后运行时会清除 `pending_feedback`，Widget 仍保持黄色正常框。
...
user loop:   interaction -> vd.state.set(patch) -> host merges into shape meta
             -> keep widget frame yellow + mark pending_feedback
...
边框仍保持黄色。显式 `vd.emit` 事件额外进入 feedback pool，并在“我的反馈”里按紫色反馈主题
