...
项目化 Widget 不是装饰图表，也不是把结论包装成互动。诊断、继续推进、局部生成、反馈回应和
阶段转换都只有在 Widget 能让学生执行一个真实判断动作时才创建：

- Discover：已有项目材料可被聚合为词频、主题、来源、时间或证据强度时，可用词云、topic
  可视化、资料时间线或证据强度标尺。没有足够材料时，用资料墙模板和 `未发现 / 待补` slot。
- Define：已有定位、用户、场景或问题陈述可被核验时，可用 HMW 检查、Persona 证据核验、
  定位清晰度量表或优先级矩阵。
- Develop：已有方案、功能、页面、组件、版本或决策记录可被比较时，可用方案筛选、信息架构
  关系图、迭代时间线、生成控制台或优先级矩阵。
- Deliver：已有交付物、导则、UI、系统架构或风险清单可被检查时，可用最终评审 rubric、
  交付准备 tracker、风险热力图或决策链追踪。

项目化 Widget 的每个数据点必须能追溯到画布对象或项目材料：`source_stage`、`source_shape_id`
或 `source_note` 至少有一个。没有来源的点只能标为 `AI 推测，需验证`，并不得作为默认高亮或
推荐。跨阶段"针灸式诊断"Widget 只能在四阶段证据已经落位后创建；它的点必须来自四阶段中的
root / slot / Widget 输出，不能用抽象标签凑数。

禁止模式：

- 没有项目证据，却创建"本项目杠杆点"散点图。
- Widget 只展示专家结论，用户没有可操作的选择、校正、筛选或提交动作。
- Widget 吞掉本应长期留在画板上的 Persona、HMW、信息架构、导则、风险清单、方案决策或
  阶段小结。
- Widget 输出没有物化路径，导致设计知识只存在于 iframe 内。
...
- **请求型 Widget 同时写 state 和 emit event**：用户点击提交时，Widget 应先
  ...
  显式处理信号。
- **交互后的视觉状态**：agent 创建的 Widget 正常态边框是黄色；只要用户在 Widget 内输入、
  改变状态或显式提交，运行时写入 `vd_widget_pending_feedback=true`，并把 Widget 边框改为
  紫色待处理态。这表示该 Widget 已产生用户反馈内容，等待下一轮 agent 处理。
- **Widget 输出属于用户反馈**：显式 `vd.emit` 会创建 `kind="widget_output"` 的反馈项，出现在
  左侧“我的反馈”入口和待处理队列中，并关联到该 Widget shape。它不进入专家“意见”流；UI 中
  按用户反馈的紫色主题呈现。
...
- **智能体回写状态**：处理请求前先用 `update_widget` 把 Widget 置为
  ...
  `update_widget` 成功后运行时会清除 `pending_feedback`，Widget 恢复黄色正常框。
...
路由规则：连续状态变化进入 snapshot，并在 agent-context 中显示 `pending_feedback`；Widget
边框转为紫色待处理态。显式 `vd.emit` 事件额外进入 feedback pool，并在“我的反馈”里按紫色反馈主题
呈现。这样可以避免 feedback pool 被交互噪音淹没，同时让“这个组件已经有用户输入”在上下文中
可见。
