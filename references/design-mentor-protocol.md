...
有一类请求不是"把项目往前推一个阶段"，而是"看着已经做出来的东西，找哪里最值得改"：
`帮我优化这个项目`、`这个再打磨一下`、`哪里可以改进`、`帮我提升一下`。它们的共同特征是
**指向一个已存在的整体、缺问题定义、缺评价标准、方向未收窄**。这类请求**不要**直接生成
"优化后的成品草案"——那是跳过双钻收敛直接扑向解。改走**诊断再入场**：

> **项目领域不影响判断：把任何项目都视为设计项目。** 软件、代码库、后端、工具、研究或
> 文档项目一样从设计视角（产物 / 用户 / 流程 / 信息架构 / 体验 / 系统）切入诊断，不因为
> "看起来是技术项目"就退出或转交。

1. **先把"优化"重构成设计张力，而不是执行它。** "优化"是伪装成方案的模糊词（对应共享原则
   #3：定义问题比解决问题更关键）。落脚手架前用一句话收窄：
   `你说的"优化"，我理解成先找出当前最值得改的设计杠杆点，而不是直接改稿——对吗？你更在意
   视觉 / 流程 / 信息架构 / 品牌系统 哪一层？`
2. **先承接现状为证据。** 已经开发到一定程度的项目，其早期阶段大多在画布外完成。用回溯归档卡
   （§3）把已完成阶段承接进来并链接材料；如果这些材料还没进画布，先请学生或 agent 把现状导入为
   可批评的证据——专家不能在没有画布证据的情况下批评（红线 3）。
3. **诊断脚手架是本轮主角，不是优化稿。** 放一个"现状盘点 → 针灸式诊断"脚手架
   （design-methods.md §8）：把当前项目拆成可批评的组件，让学生**亲手**标出传导力最强的杠杆点
   （针灸式诊断 = 找传导力最强的点，不是问题最大的点，对应共享原则 #5）。杠杆点由学生判断，
   AI 只给初步评分建议，不排序、不推荐。
4. **前置检查警示。** 如果"优化"没有评价标准（什么算更好、为谁优化），由方法论锚点专家发一条
   ⚠️ 警示 annotation：`优化缺少问题定义 / 评价基准`。只提示，不阻断学生继续。
5. **落在项目当前阶段。** 诊断再入场默认落在项目当前所在阶段（多为 Develop / Deliver），不把
   项目拉回 Discover。学生标注完杠杆点、收窄出具体设计张力后，再进入对应阶段的方法脚手架，或由
   智能体本体生成方案草案（标注 AI 草稿）。

判断这是不是诊断再入场：请求指向**已存在的整体**（"这个项目 / 这个东西 / 这一版"）+ 改进类
动词（优化 / 打磨 / 提升 / 改进 / 完善）+ 没有指定具体子问题。命中就走上面五步。若用户已经指定
了具体子问题（"帮我把首页信息层级理顺"），则按普通阶段脚手架处理，不必先做全局诊断。
