...
画布只放"说明+提问+留白"、样式字段、框架级画布图库）。**没做的是 case 的四个协同亮点
A/E/F/G**（前端为主），对应 `examples/test-usecase-playable-city.md` 里标 🔨 的验收点。
...
## 剩余工作：四个亮点 A / E / F / G（前端为主）

### A — 脚手架归属"连线到左侧专家 bar"（方向已定，未实现）
- **目标**：不在 frame 上放头像；改为 hover/选中左侧专家 bar 里的某专家 → 画虚线连到该专家的
  脚手架 frame（并高亮 frame）。悬停使用说明（`vd_usage_note`）可作为次要项后加。
...
- **可直接复用**：
  ...
  - `refreshHtmlComponents`（:3742）在相机变化时重算 overlay 列表——把 scaffold→expert 的连线数据
    也在这里刷新。
- **要新增**：给 `ActiveExpertsDock`（:2143）里每个专家 avatar 一个 ref（现在只有 `overviewButtonRef`）；
  `selectedExpertName`（:3778）/ hover 时，找出该专家的所有 scaffold frame（遍历 shapes，
  `meta.vd_method_source.experts` 含该名），用该专家 avatar 元素当 source 画 FeedbackConnectorOverlay。
- **参考实现骨架**（我回退前写的 extractor，可重建）：遍历 `getCurrentPageShapes` → 过
  `isAgentScaffoldRootShape` → 取 `meta.vd_method_source.experts`（`normalizeExpertCandidate`）+
  `pagePointToViewport(topLeft)`，得到 `{shapeId, x, y, experts, usageNote}` 列表，供连线用。
