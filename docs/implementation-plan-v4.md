...
新建画布默认保持空白，不预制 agent 工作区、用户反馈区或共享决策区；需要结构时由用户
创建 section，或由画布工具栏中的脚手架库 / agent 显式投放合适的协作脚手架。暂不实现多人实时协作、
复杂矢量设计工具能力和完整素材管理系统。
...
- 新建画布不生成默认 seed node；脚手架可以按任务需要生成角色区域、灵感墙或工作区。
...
- 画布 MVP 在画布工具栏中提供项目级脚手架库；脚手架不仅是空模板，也可以包含初始内容、交互槽位、widget 和下一步动作。
- Widget 第一版采用透明背景的 HTML iframe + JSON 状态/schema 元数据，不做完整 widget SDK；尺寸遵循内部内容的 min/max sizing contract。
- 补全请求第一版作为画布工具栏里的紫色矩形工具，生成发光矩形区域 + 需求描述，并写入 `semantic_index.completion_requests`；选区重构请求通过普通批注处理。
- 画布提供普通/来源高亮预览模式：用户内容黄色，智能体内容紫色，混合内容蓝色。
- 画布保存时生成语义 diff，记录用户或智能体新增、修改、移动/缩放、删除了哪些语义节点。
- 智能体每次改画布前必须读取 `/api/canvas-workspaces/{id}/context`，检查 snapshot、semantic index、events、open feedback 和 open completion requests。
- 脚手架写入后必须执行 layout review gate：检查重叠、越界、可读尺寸、sticky note 类型和语义一致性；具备浏览器/截图能力时，还要查看聚焦截图并在发现布局问题后自动调整再保存。
- 画布 section 或节点可以作为正式 artifact 引用到后续汇报或文档。
